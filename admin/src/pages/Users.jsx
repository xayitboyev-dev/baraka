...
export default function Users() {
  ...
  return (
    <div className="users flex flex-col gap-6 p-10">
      ...
      <div className="filter flex gap-4">
        ...
        <button onClick={fetchData} className="flex gap-1 cursor-pointer items-center bg-blue-600 text-white rounded-xl px-6 py-2">Filter <RefreshCcw size={16} className="ml-2" /></button>
      ...
      <div className="flex flex-col gap-2 max-w-xl">
        {loading ? (
          ...
          users?.length ? (
            users.map((user) => (
              <div key={user.id} className="flex items-center gap-4 py-2 px-6 border border-slate-200 shadow rounded-xl">
                <span className='text-gray-500 w-14'>#{user.orderId}</span>
